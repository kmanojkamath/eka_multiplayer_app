...
class HostGamePlay {
  ...
  CardColor get selectedColor => cardStorage.selectedColor.value;
  ...
  void changeColor(CardColor color) {
    cardStorage.selectedColor.value = color;
  }
  ...
  Future<Move> gameStart() async {
    ...
    final log = GameStartLog(1, playerHands: P, topCard: deckPile.last);
    await sendLog(log);
  ...
  bool isPlayable(int ci) {
    ...
    return (top.isWild && (card.color == selectedColor || card.isWild)) ||
        card.isWild ||
  ...
  Future<Move> drawForHost() async {
    ...
    if (isPlayable(ci)) {
      ...
      cardAnimations.unshowPlayableCards(didPlay: false);
  ...
  Future<Move> processLog() async {
    ...
    switch (log) {
      ...
      case PlayerPlayLog(
        ...
        playerNPile(playerNumber).remove(putCard);
...
class PlayerNGamePlay {
  final CardAnimations cardAnimations;
  final DocumentReference roomRef;
  final int currentPlayer;
  final int startingPlayer;

  const PlayerNGamePlay(
    this.currentPlayer, {
  ...
  int localPlayerNumber(int playerNumber) {
    return (playerCount + playerNumber - currentPlayer) % playerCount;
  }

  int cloudPlayerNumber(int playerNumber) {
    return (playerNumber + currentPlayer) % playerCount;
  }
  ...
  Future<Move> gameStart() async {
    turn = localPlayerNumber(startingPlayer);

    final log = await waitForNextLog() as GameStartLog;

    List<List<int>> P = List.generate(playerCount, (i) => []);

    for (int i = 0; i < playerCount; i++) {
      P[i] = log.playerHands[localPlayerNumber(i)];
      deckPile.removeWhere((element) => P[i].contains(element));
    }

    for (int j = 0; j < 7; j++) {
      for (int i = 0; i < playerCount; i++) {
        int t = (i + localPlayerNumber(startingPlayer)) % playerCount;
        int ci = P[t][j];
        playerNPile(t).add(ci);
        if (t != 0) {
          await cardAnimations.playerNDrawCard(ci, t);
    ...
    debugPrint(localPlayerNumber(startingPlayer).toString());

    return Move.values[localPlayerNumber(startingPlayer)];
  }
  ...
  Future<Move> changeTurn(int nextPlayer, {bool? isReverse}) async {
    final log = ChangeTurnLog(
      nextLog,
      nextPlayer: cloudPlayerNumber(nextPlayer),
      isReverse: isReverse ?? false,
    );
  ...
  Future<Move> drawForPlayer() async {
    if (deckPile.isEmpty) reshuffle();
    ...
    final log = PlayerDrawLog(
      nextLog,
      playerNumber: cloudPlayerNumber(0),
      drawCards: [ci],
    );
    await sendLog(log);
  ...
  Future<Move> playerTurn() async {
    ...
    if (selectedCard < 0) return drawForPlayer();
    ...
    if (playerNPile(0).isEmpty) {
      final log = GameWinLog(nextLog, playerNumber: cloudPlayerNumber(0));
      await sendLog(log);
      cardStorage.winner = cloudPlayerNumber(0);
      return Move.gameWin;
    }
    ...
    if (topCard.isDrawTwo) {
      ...
      final log = PlayerDrawLog(
        nextLog,
        playerNumber: cloudPlayerNumber(prey),
        drawCards: [ci1, ci2],
      );
  ...
  Future<Move> processLog() async {
    ...
    switch (log) {
      case ChangeTurnLog(
        ...
        turn = localPlayerNumber(nextPlayer);
        if (isReverse) reverse();
        return Move.values[localPlayerNumber(nextPlayer)];
      ...
      case PlayerDrawLog(
        ...
        for (int i = 0; i < drawCards.length; i++) {
          deckPile.remove(drawCards[i]);
          playerNPile(localPlayerNumber(playerNumber)).add(drawCards[i]);
      ...
      case PlayerPlayLog(
        ...
        if(color!=null) changeColor(color);
        playerNumber = localPlayerNumber(playerNumber);
        topCard = putCard;
        playerNPile(playerNumber).remove(putCard);
        await cardAnimations.playerNPlayCard(playerNumber);
        return Move.values[turn];
