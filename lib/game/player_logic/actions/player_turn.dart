...
extension PlayerTurn on PlayerGamePlay {
  Future<Move> _drawForPlayer() async {
    ...
    final log = PlayerDrawLog(
      _nextLog,
      playerNumber: _cloudPlayerNumber(0),
      drawCards: [ci],
    );
    await _sendLog(log);
  ...
  Future<Move> playerTurn() async {
    ...
    if (_playerNPile(0).isEmpty) {
      final log = GameWinLog(_nextLog, playerNumber: _cloudPlayerNumber(0));
      await _sendLog(log);
      _cardStorage.winner = _cloudPlayerNumber(0);
      return Move.gameWin;
    }
    ...
    if (_topCard.isDrawTwo) {
      ...
      final log = PlayerDrawLog(
        _nextLog,
        playerNumber: _cloudPlayerNumber(prey),
        drawCards: [ci1, ci2],
      );
